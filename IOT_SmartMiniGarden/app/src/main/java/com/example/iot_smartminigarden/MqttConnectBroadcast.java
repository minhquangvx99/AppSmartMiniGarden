...
import com.example.iot_smartminigarden.mqtt.*;
...
import static com.example.iot_smartminigarden.config.Config.TOPIC_WARNING;

public class MqttConnectBroadcast extends BroadcastReceiver implements InterfaceMqttController {
                ...
                if (topic.equals(TOPIC_WARNING)) {
                    if (json.has("code") && json.has("message")) {
                        if (json.getInt("code") == 113) {
                            NotificationSmartMiniGarden.callNotify(mContext, idNotify++, NOTIFI_ID, "Warnning", "Có người đang cố ý mở của nhà bạn");
                        } else if (json.getInt("code") == 114) {
                            NotificationSmartMiniGarden.callNotify(mContext, idNotify++, NOTIFI_ID, "Warnning", "Không khí trong nhà bạn có gì đó không ổn");
                        } else if (json.getInt("code") == 110) {
                            if (json.getString("message").equals("failed"))
                                NotificationSmartMiniGarden.callNotify(mContext, idNotify++, NOTIFI_ID, "Thêm thẻ", "Thêm thẻ thất bại");
                            else if (json.getString("message").equals("success"))
                                NotificationSmartMiniGarden.callNotify(mContext, idNotify++, NOTIFI_ID, "Thêm thẻ", "Thêm thẻ thành công");
                        }
                    }
                }
    ...
    public void onReceive(Context context, Intent intent) {
        mContext = context;
        NotificationSmartMiniGarden.createChannel(context, NOTIFI_ID, NOTIFI_ID, NotificationManager.IMPORTANCE_HIGH);
        initMqtt();
    }
}
